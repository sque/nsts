...
class BasicTerminal(ClientTerminal):
    ...
    def welcome(self):
        print "Network SpeedTest Suite [NSTS] Version" \
            " {version[0]}.{version[1]}.{version[2]}".format(
    ...
    def client_connected(self, connection):
        if self.options['numerical_addr']:
            ...
            local = [socket.gethostbyaddr(
                connection.socket.getsockname()[0])[0],
        ...
        if self.options['suite_filename']:
            print "Suite             : {0}".format(
                self.options['suite_filename'])
