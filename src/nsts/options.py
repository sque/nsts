...
class UnknownOptionError(OptionError):
    ...
    def __init__(self, opt_id):
        super(UnknownOptionError, self).__init__(
            "There is no option with id '{0}'".format(opt_id))
...
class OptionsDescriptor(object):
    ...
    def add_option(self, option_id, help, unit_type, default=None):
        ...
        '''
        self.__supported[option_id] = OptionValueDescriptor(
            option_id, help, unit_type, default)
...
class Options(object):
    ...
    def __init__(self, descriptor, values={}):
        if not isinstance(descriptor, OptionsDescriptor):
            raise TypeError(
                "{0} is not instance of OptionsDescriptor".format(descriptor))
